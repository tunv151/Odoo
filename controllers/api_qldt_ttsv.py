...
api_url = Route('student_payment', version='1', app='qldt')

class QLDTttsv(Controller):

    @route(route=api_url, methods=['POST'], auth='public', type='json')
    def ttsv(self):
        try:
            verify = ["student_payment_id", "code", "unit_code", "student_code"]
            ...
            if action == 'delete':
                ...
                if not business_unit:
                    ...
                    ttsv_id = request.env['hp.thanh.toan.sinh.vien'].sudo().search([
                        ('so_chung_tu', '=', qldt_id),
                    ...
                    if not ttsv_id:
                        code = '147'
                        message = f'Thanh toán sinh viên (ID QLDT: {qldt_id}) không tồn tại trong hệ thống'
            ...
            if code == '000':
                # Tạo Log Sync
                log_sync = request.env['log.sync.receive.ttsv'].sudo().create({
                ...
                if res_code == '000':
                    return Response.success('Đồng bộ thanh toán sinh viên thành công', data={'code': res_code}).to_json()
                else:
            ...
            _logger.error("API QLDT TTSV Critical Error: %s", str(e), exc_info=True)
